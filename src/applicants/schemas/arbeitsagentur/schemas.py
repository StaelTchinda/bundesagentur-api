...
class Lokation(BaseModel):
    ...
    @classmethod
    def get_matching_locations(cls, location: Union[Text, int], radius: Optional[int] = None) -> List["Lokation"]:
        if location is None:
            raise ValueError("Location is None")
        
        main_localities: List[Locality]
        if isinstance(location, int) or (isinstance(location, Text) and location.isdigit()):
        ...
        if radius is not None:
            for locality in main_localities:
                radius_localities.extend(get_localities_in_radius(CountryCode.DE, locality.postalCode, radius))
    ...
    @classmethod
    def _build_basic_location_filter_(cls, location: "Lokation", location_keyword: Text) -> bool:
        if location.ort is not None and re.search(location_keyword, location.ort, re.IGNORECASE):
            return True
        elif location.plz is not None and re.search(location_keyword, str(location.plz), re.IGNORECASE):
            return True
        elif location.region is not None and re.search(location_keyword, location.region, re.IGNORECASE):
            return True
        elif location.land is not None and re.search(location_keyword, location.land, re.IGNORECASE):
            return True
        else:
            return False
        
    @classmethod
    def build_location_filter(cls, location_keyword: Text, location_radius: Optional[int] = None) -> (Callable[["Lokation"], bool]):
        if location_radius is None:
            return lambda location: cls._build_basic_location_filter_(location, location_keyword)
        else:
            matching_locations = cls.get_matching_locations(location_keyword, location_radius)
            matching_location_orts = set([location.ort for location in matching_locations])
            matching_location_plzs = set([location.plz for location in matching_locations])
            matching_location_regions = set([location.region for location in matching_locations])
            logger.info(f"Matching locations: {matching_location_orts}, {matching_location_plzs}, {matching_location_regions}")
