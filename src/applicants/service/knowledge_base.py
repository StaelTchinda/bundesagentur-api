...
class KnowledgeBaseDb:
    ...
    def get_all(self) -> List[Text]: 
        ...
        """
        results: List[Document] = self.db.all()
        records: List[Text] = [str(record) for record in results]
        return records

    
    def get(self, query: QueryLike) -> List[Text]:
        """
        ...
        """
        results: List[Document] = self.db.search(query)
        records: List[Text] = [str(record) for record in results]
        return records

    
    def get_by_regex(self, regex: Text) -> List[Text]:
        """
